...
DELETE FROM signals.signals WHERE signal_id = '2';
-- ALTER TABLE `signals`.`signals` 
-- ADD COLUMN `datetime` TEXT NULL DEFAULT NULL AFTER `user_id`;
SELECT * FROM signals.users;
INSERT INTO signals.signals (signal_id, signal_name, signal_description, user_id, datetime) VALUES (2, 2, 2, 11, datetime);
SELECT * FROM signals.signals;